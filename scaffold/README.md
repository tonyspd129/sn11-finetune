...
(the new-mechanism design — what miners submit and how it's evaluated).
...
python -m scaffold.example_miner
```
...
`scaffold`). There's nothing to wire up — the harness always loads its own files, the same
...
`PromptContext` (`task`, `base_system`, `skills`, `tool_specs`). See
...
workspace `shell`, `workdir`, `config` (operator wiring), and `skill_lookup`.
...
- **No network tools.** The harness ships only filesystem / shell / skill tools — there is no
  `web_fetch` / `web_search`. Egress is banned at the **sandbox/network layer** (season1: network
  fully off), which is the real boundary anyway since the agent could otherwise reach out via
  `execute_command`.
...
| `toolkit.py` | `Tool` contract, `ToolManager`, `Shell`, helpers (`truncate`, `resolve_path`) |
...
| `tools/` | the 6 default tool plugins (shell · fs view/edit/write/grep · load_skill) |
